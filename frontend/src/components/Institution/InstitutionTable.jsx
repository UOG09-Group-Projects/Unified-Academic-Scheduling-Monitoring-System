...
const InstitutionTable = ({ institutions, onEdit, onDelete, onApprove, onReject }) => {
  if (!institutions.length) {
    return <EmptyState icon={Building2} title="No institutions found" />;
  }

  const showActions = Boolean(onEdit || onDelete || onApprove || onReject);
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-ink/[0.02]">
            {['Institution', 'Owner', 'Email', 'Status', 'Created'].map((h) => (
            ...
            {showActions && <th className="px-6 py-3 w-32 border-b border-ink/[0.06]" />}
          </tr>
        </thead>
        <tbody className="divide-y divide-ink/[0.05]">
          {institutions.map((inst, i) => {
            const statusBadge = STATUS_BADGE[inst.status] ?? STATUS_BADGE.APPROVED;
                ...
                {showActions && (
                  <td className="px-6 py-3">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {isPending && onApprove && (
                        <button
                          onClick={() => onApprove(inst)}
                          className="w-8 h-8 rounded-lg flex items-center justify-center text-ink-faint hover:text-success hover:bg-emerald-50 transition-colors"
                          aria-label="Approve institution"
                        >
                          <Check size={14} />
                        </button>
                      )}
